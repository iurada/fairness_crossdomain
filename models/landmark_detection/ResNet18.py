...
import copy
from models.landmark_detection.MLP import MLP
...
class PoseResNetDANN(nn.Module):
    """
    `Simple Baseline <https://arxiv.org/abs/1804.06208>`_ for keypoint detection.
    ...
    def __init__(self, backbone, upsampling, feature_dim, num_keypoints, finetune=False):
        ...
        self.discriminator = MLP(self.backbone.out_features, 2, 0.5)
        self.update_count = 0
...
def pose_resnet18_DANN(image_size, num_keypoints, pretrained_backbone, deconv_with_bias=False, finetune=False, progress=True, **kwargs):
    backbone = resnet18(pretrained_backbone, progress)
    upsampling = Upsampling(backbone.out_features, hidden_dims=(image_size, image_size, image_size), bias=deconv_with_bias)
    model = PoseResNetDANN(backbone, upsampling, image_size, num_keypoints, finetune)
    return model