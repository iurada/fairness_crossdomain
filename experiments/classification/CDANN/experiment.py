...
class Experiment:
    ...
    def additional_arguments(parser):
        parser.add_argument('--cdann_grad_penalty', type=float, default=0.01)
        parser.add_argument('--cdann_d_steps_per_g_step', type=int, default=1)
        parser.add_argument('--cdann_lambda', type=float, default=1.0)
        return ['cdann_grad_penalty', 'cdann_d_steps_per_g_step', 'cdann_lambda']
    ...
    def train_iteration(self, data):
        ...
        disc_input = z + self.model.class_embeddings(y)
        ...
        disc_loss += self.args.cdann_grad_penalty * grad_penalty

        d_steps_per_g = self.args.cdann_d_steps_per_g_step
        if (self.model.update_count % (1 + d_steps_per_g) < d_steps_per_g):
            ...
            gen_loss = (classifier_loss + (self.args.cdann_lambda * -disc_loss))
            self.disc_opt.zero_grad()
